...
export function normalizeAppTags(tags: Iterable<string | null | undefined>): string[] {
...
function tokenizeMetadata(value: string | null | undefined): string[] {
  if (!value?.trim()) {
    return []
  }
...
export function suggestAppTagsFromListing(input: {
  ...
  for (const v of [input.scope, input.productType, input.vertical, input.rawCategoryHint]) {
    if (v) {
...
export function popularTagsFromAllAssignments(
  allTagLists: ReadonlyArray<readonly string[]>,
  limit: number,
): string[] {
