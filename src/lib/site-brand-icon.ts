...
function collectSiteLogoPathCandidates(baseUrl: string): Array<IconCandidate> {
  const paths = [
    "/logo.svg",
    "/logo.png",
    "/logo.webp",
...
function collectLinkedIconCandidates(
  ...
  $("link[rel][href]").each((_, element) => {
    ...
    if (!/\b(icon|apple-touch-icon|mask-icon|fluid-icon)\b/.test(relLower))
      return;
...
function collectConventionalFallbackCandidates(
  baseUrl: string,
): Array<IconCandidate> {
...
function dedupeCandidates(
  candidates: Array<IconCandidate>,
): Array<IconCandidate> {
  ...
  return [...bestByUrl.values()].toSorted(
    (left, right) =>
...
async function probeImageUrl(
  ...
  try {
    ...
    const isImage =
      isImageByType ||
      (isPotentialImageByUrl &&
        !bufferLooksLikeHtml(body) &&
        (sniffBinaryImage || pathLooksLikeIco)) ||
