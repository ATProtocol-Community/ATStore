...
export type ListingMentionIndex = {
  ...
  listings: Array<ListingMentionIndexRow>;
};
...
export function extractUrlsFromText(text: string): Array<string> {
  const out: Array<string> = [];
  const re = /https?:\/\/[^\s<>"{}|\\^`[\]]+/gi;
  let m: RegExpExecArray | null;
  while ((m = re.exec(text)) !== null) {
    const u = m[0].replaceAll(/[),.;:]+$/g, "");
    out.push(u);
  }
  return out;
}
 ...
 */
export function extractAtHandleMentionsFromText(text: string): Array<string> {
  const out: Array<string> = [];
  ...
  while ((m = re.exec(text)) !== null) {
    const capture = m[1];
    if (!capture) continue;
    const raw = capture.replaceAll(/[),.;:]+$/g, "").trim();
 ...
 */
export function extractBskyAppProfileHandlesFromUrls(
  urls: Array<string>,
): Array<string> {
  const out: Array<string> = [];
  const seen = new Set<string>();
...
export function buildListingMentionIndex(
  rows: Array<ListingMentionIndexRow>,
): ListingMentionIndex {
...
export function facetLinkUris(facets: FacetSlice[] | undefined): Array<string> {
  if (!facets?.length) return [];
  const out: Array<string> = [];
  for (const facet of facets) {
