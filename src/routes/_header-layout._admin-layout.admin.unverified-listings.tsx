...
function formatRejectionDate(iso: string) {
...
interface ListingReviewCardProps {
  row: UnverifiedRow | ResubmittedRow;
  busy: string | null;
  setBusy: (id: string | null) => void;
  rejectDraft: Record<string, string>;
  setRejectDraft: React.Dispatch<React.SetStateAction<Record<string, string>>>;
...
function ListingReviewCard({
  ...
  return (
    <Card style={styles.card} size="lg">
      {row.heroImageUrl ? (
        <CardImage aspectRatio={16 / 9} src={row.heroImageUrl} alt="" />
      ) : null}
      <CardBody>
        <Flex direction="column" style={styles.cardBody}>
          {priorRejection ? (
            <Alert
              variant="warning"
              title={`Previously rejected ${formatRejectionDate(priorRejection.rejectedAt)}`}
              style={styles.priorRejectionAlert}
            >
              <RestrictedMarkdownContent
                compact
                content={priorRejection.reason}
            ...
            <Flex gap="xl" align="start">
              ...
              <Flex direction="column" gap="xl">
                ...
                <Text size="sm" weight="bold">
                  {row.categorySlugs?.[0] ? `${row.categorySlugs[0]}` : ""}
                </Text>
                <Flex wrap gap="xl">
                  {row.externalUrl ? (
                    <AriaLink
          ...
          <Flex direction="column" gap="sm" style={styles.rejectNotes}>
            <TextArea
  ...
  return (
    <Page.Root variant="large" style={styles.page}>
      <Flex direction="column" gap="6xl" style={styles.section}>
        ...
        {data.resubmittedAfterRejection.length > 0 ? (
          <Card>
            <CardHeader>
              <CardTitle>
                Resubmitted after rejection (
                {data.resubmittedAfterRejection.length})
              </CardTitle>
            </CardHeader>
            <CardBody>
              <SmallBody>
                Owners updated their record after a prior rejection. Re-check
                against your earlier feedback and approve or reject again.
              </SmallBody>
              <Flex direction="column" style={styles.listStack}>
                {data.resubmittedAfterRejection.map((row) => (
                  <ListingReviewCard
                    key={row.id}
                    row={row}
                    busy={busy}
                    ...
                    refresh={refresh}
                    priorRejection={{
                      reason: row.lastRejectionReason,
                      rejectedAt: row.lastRejectionAt,
                    }}
                  />
                ))}
              </Flex>
