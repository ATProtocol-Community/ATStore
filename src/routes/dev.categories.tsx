...
function DevCategoriesPage() {
  ...
  const stats = useMemo(() => {
    const assigned = listings.filter(
      (listing) => listing.categorySlug !== null,
    ).length;
  ...
  const uncategorizedListings = useMemo(
    () => listings.filter((listing) => listing.categorySlug === null),
    [listings],
  );
  const categorySuggestions = useMemo(
    () =>
      collectStructuredDirectoryCategorySuggestions(
        listings.map((listing) => listing.categorySlug),
      ),
    [listings],
  );
  async function saveListing(listing: DirectoryListingCategoryAssignment) {
    const draft =
      ...
      parseStructuredDirectoryCategory(listing.categorySlug);
  ...
  function updateDraft(
    ...
    setDraftCategories((current) => {
      const existing =
        current[listingId] ??
        parseStructuredDirectoryCategory(
          listings.find((listing) => listing.id === listingId)?.categorySlug ??
            null,
        );
          ...
          <Flex direction="column" style={styles.pageContent}>
            ...
            <Flex direction="column" style={styles.pageHeader}>
              ...
              <Body style={styles.helperText}>
                ...
                still use parent nodes like Apps, Bluesky, or Protocol when a
                more specific child category does not exist yet.
              </Body>
            </Flex>
            ...
            <Flex direction="column" style={styles.listingList}>
              {uncategorizedListings.length > 0 ? (
                uncategorizedListings.map((listing, index) => {
                  ...
                  const isDirty = nextCategorySlug !== listing.categorySlug;
                                ...
                                >
                                  <Flex direction="column" gap="sm">
                                    ...
                                    {candidate.listing.externalUrl ? (
                                      <Link
                                        href={candidate.listing.externalUrl}
                                      >
                                        Open candidate URL
                                      </Link>
                                    ) : null}
                        ...
                        <div {...stylex.props(styles.listingFields)}>
                          ...
                          {draft.kind.trim().toLowerCase() === "protocol" ? (
                            <ComboBox
                              ...
                              items={
                                categorySuggestions.protocolCategoryOptions
                              }
                        ...
                        <Flex gap="lg" style={styles.listingFooter}>
                          ...
                          >
                            ...
                            <AlertDialogDescription>
                              ...
                              <strong>{listing.name}</strong> from the
                              directory.
                            </AlertDialogDescription>
                            <AlertDialogFooter>
                              <AlertDialogCancelButton />
