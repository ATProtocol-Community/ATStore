...
type ScriptArgs = {
  ...
  id: string | null;
  input: string | null;
  /** When set, skip Playwright capture and use this image as the Gemini reference. */
...
function parseArgs(argv: string[]): ScriptArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--screenshot" || arg === "-s") {
      out.screenshotPath = argv[++i] ?? null;
      continue;
    }
...
Options:
  ...
  -s, --screenshot <path>  Use a local PNG/JPEG/WebP instead of capturing the listing URL (hero mode only)
  -h, --help          Show help
`);
}
  ...
  for (const extension of [".png", ".jpg", ".webp"]) {
    try {
      await stat(
        path.resolve(
          GENERATED_IMAGE_DIR,
          `${listing.assetBaseName}${extension}`,
        ),
      );
...
async function readManualListingRecords(
  inputPath: string,
...
function buildPageScreenshotPath(
  listing: CandidateListing,
  url: string,
): string {
...
function buildGeneratedImageTarget(
  listing: CandidateListing,
  mimeType: string,
): {
  absolutePath: string;
...
- Prefer calm, structured compositions over decorative motion graphics—no whispy streaks, light trails, or swooshing energy ribbons.
...
- If the listing is dev focused and doesnt have much branding to work with, use this fallback style:
  ...
  - Use soft 3D gradients, glossy lighting, rounded cards, translucent glass layers, and subtle depth—calm and grounded, not kinetic.
  - Composition: wide 16:9 banner with clear hierarchy; decorative elements stay minimal and structural (panels, cards, soft blobs)—not atmospheric.
  - Layer foreground, midground, and background with UI-like depth; avoid abstract swooshes, light trails, whispy energy streaks, particle streams, or flowing ribbon shapes.

Constraints:
- No whispy energy streaks, light trails, swooshes, aurora ribbons, particle streams, or other decorative motion graphics.
...
function mimeTypeFromScreenshotPath(filePath: string): string {
...
async function captureReferenceScreenshot(
  ...
  if (screenshotOverride?.trim()) {
    const absolutePath = path.resolve(process.cwd(), screenshotOverride.trim());
    const buffer = await readFile(absolutePath);
    const filePath = buildPageScreenshotPath(
      listing,
      `${url}#file:${path.basename(absolutePath)}`,
    );
    await writeFile(filePath, buffer);
    return {
      buffer,
      filePath,
      mimeType: mimeTypeFromScreenshotPath(absolutePath),
    };
  }
...
async function generateMarketingImage(
  ...
  for (let attempt = 1; attempt <= IMAGE_REQUEST_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Gemini image request failed: ${await response.text()}`,
        );
      }
...
async function getCandidateListings(
  args: ScriptArgs,
...
async function processListing(
  ...
  if (!pageUrl) {
    console.log(
      `Skipping ${listing.name} (${listing.id}) because it has no usable URL.`,
    );
    return;
  }
  ...
  if (args.screenshotPath) {
    console.log(`Using reference screenshot ${args.screenshotPath}`);
  }
  ...
  const {
    ...
    mimeType: screenshotMimeType,
  } = await captureReferenceScreenshot(listing, pageUrl, args.screenshotPath);
  ...
  if (args.dryRun) {
    console.log(
      `Dry run enabled; leaving database row unchanged for ${listing.name}.`,
    );
    return;
  }
...
async function processListingIcon(
  ...
  console.log(
    `Processing icon for ${listing.name} (${listing.id}) -> ${pageUrl}`,
  );
